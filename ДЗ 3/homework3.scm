...
(define (derivative expr)
  (if (not(list? expr))
      ...
      (cond
        ...
        ((and (equal? (car expr) '*) (number? (cadr expr)))
         (if (< (length expr) 4)
        ...
        ((equal? (car expr) 'cos) (list '* (list '-sin (cadr expr)) (derivative (cadr expr))))
        ...
        ((equal? (car expr) '*) (list '+ (list '* (derivative (cadr expr)) (caddr expr))
                                      (list '* (cadr expr) (derivative (caddr expr)))))
        ((not(null? (sign expr))) (list (sign expr) (derivative (cadr expr)) (derivative (caddr expr)))))))
